...
def get_credentials():
    ...
    if missing_vars:
        raise f"Error: Missing required environment variables: {', '.join(missing_vars)}"
...
def run_suiteql_query(query, limit=None, offset=None):
    ...
    try: 
        response = requests.post(url, params=params, auth=oauth, headers=headers, json=payload)
